...
var mostWatermelon =
    (
    from p in products
    where p.ProductName == "Pastèques"
    orderby p.Quantity
    select p.Producer
    ).First();

Console.WriteLine($"Il y a {nbrPeach} vendeurs de pêches");
Console.WriteLine(($"C'est {mostWatermelon} qui a le plus de pastèques"));
Console.ReadLine();